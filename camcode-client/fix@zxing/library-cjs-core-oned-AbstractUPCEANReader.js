    ...
    */
    AbstractUPCEANReader.findStartGuardPattern = function (row) {
        ...
        while (!foundStart) {
            ...
            if(!foundStart && nextStart - start > 20) nextStart=row.getNextUnset(start)   //mg 12.2021 start=XX__XX allow false XX start, white big space
        }
        return startRange;
    };
            ...
            if (row.get(x) !== isWhite) {
                ...
                if (counterPosition === patternLength - 1) {
                    ...
                    for (var i = 0; i < counterPosition - 1; i++) {
                        counters[i] = counters[2+i]   //slice[i]; mg 12.2021 slice above wrong (-1 should remove)
                    }
